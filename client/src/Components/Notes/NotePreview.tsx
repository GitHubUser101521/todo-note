...
function NotePreview({ _id, title, createdAt, content, category, color }: NoteType) {
    ...
    const handleNoteEdit = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
      e.stopPropagation()
  ...
  return (
    <div onClick={handleNoteNavigation} className={`note-preview ${color === 'default' ? 'border-2' : color}`}>
      <p className='font-bold text-xl'>{ title }</p>
      <p 
        className='text-xs text-gray-700'
      >
        { convertToDate(createdAt) } { category === 'All' ? '' : `| ${category}`}
      </p>
      ...
      <div className='hover-icons absolute top-8 right-8'>
        <button 
          className='bg-red-500 w-6 h-6 rounded-full place-items-center mr-2 hover:brightness-75'
        ...
        <button 
          className='bg-blue-400 w-6 h-6 rounded-full place-items-center hover:brightness-75'
          onClick={(e) => handleNoteEdit(e)}
        >
          <MdEditSquare />
        </button>
      </div>
